    ...
    def sort(self):
        ...
        while self.light_is_on():  # create while loop based on flag
            ...
            while self.can_move_right():  # check items to right, #keep repeating steps below, until can't move right
                ...
                if self.compare_item() == 1:  # if item in hand < item in list position
                    self.swap_item()  # if true, swap
                    self.set_light_on()  # since swap occured, turn flag on

                self.move_left()  # move to left
            ...
            while self.can_move_left():  # repeat the steps above for items to left, once can't move right. repeat steps below until can't move left 
                ...
                if self.compare_item() == -1:  # if item in hand > item to the left list position
                    self.swap_item()  # swap
                    self.set_light_on()  # turn while flag  om

                self.move_right()  # move right
                self.swap_item()  # pick up item
                self.move_left()  # move left to compare the 2 items
